...
1. 횡방향 제어는 본질적으로 **2-입력 MIMO 문제**이다 — 능동 전륜 조향(AFS) `δ`와 직접 요모멘트(DYC/ESC) `M_z`. 입력 간 결합을 수동으로 튜닝하는 PID(SISO 루프 4개)보다, 비용함수로 최적 배분을 한 번에 푸는 LQR이 이론적으로 정합적이다 [3].
...
상태 `x = [β, r]^T`(차체 슬립각, 요레이트), 입력 `u = [δ, M_z]^T`(전륜 조향, 요모멘트):
...
정상상태 요추종 오차를 제거하기 위해 적분 상태 `ξ`를 추가한다:
...
목표 요레이트 `r_ref`는 제공된 `calc_ref_yaw_rate`(선형 bicycle 정상상태)로 driver 조향각으로부터 계산된다.
...
**설계 목표**: yaw rate 추종(overshoot ≤10%, settling ≤0.8 s) + |β| 한계 억제(스핀아웃 방지).

**구조 — 두 LQR의 β-블렌딩.** 단일 LQR로 모든 영역을 처리하면 한계영역용 강한 슬립각 페널티가 정상주행(폐루프 운전자 추종)을 과도하게 간섭한다. 따라서 슬립각 게이트 `σ(|β|)`로 두 설계를 부드럽게 섞는다:
...
- **K_nom (정상영역, steer-only)**: 약한 슬립각 페널티의 부드러운 요추종. 운전자와 충돌하지 않음.
- **K_lim (한계영역, 2-입력)**: 강한 슬립각 페널티 + ESC 요모멘트. 한계에서만 개입.

**Q/R 선택 및 게인 산정.** LQR 가중을 다음과 같이 선택하였다(`diag` 성분):

| 영역 | q_β | q_r | q_ξ | r_δ | r_Mz |
|---|---|---|---|---|---|
| K_lim (한계) | 1500 | 50 | 60 | 1.0 | 2×10⁻⁵ |
| K_nom (정상) | 5 | **2** | 200 | 0.4 | — |

- 한계영역 `q_β=1500`: A7 스핀아웃(β→30°) 억제가 최우선이므로 슬립각을 지배적으로 페널티.
- `r_Mz=2×10⁻⁵`: 요모멘트는 N·m 스케일(수천)이라 입력비용을 작게 정규화해 조향과 균형.
...
의 안정(음의 실수부) 고유벡터 `[X_1; X_2]`로부터 `P = \mathrm{Re}(X_2 X_1^{-1})`, `K = R^{-1}B_\text{aug}^T P`로 계산한다. `eig`만 사용하므로 toolbox 없이도 동작한다. **Gain scheduling**: `v_x` 변화 시(임계 0.5 m/s) `K_nom, K_lim`을 재계산·캐싱한다.
...
반능동 댐퍼는 에너지를 소산만 할 수 있고(`F = c·(stroke rate)`, `c∈[c_min,c_max]`), 정통 skyhook은 차체 절대속도를 필요로 한다. 그러나 본 환경의 상태에서는 차체 **heave 절대속도가 관측 불가**(runner의 `suspState.zs_dot`는 롤/피치 모달 성분만 재구성). 따라서 관측 가능한 신호로 다음을 결합한다:

$$ c_i = \mathrm{clip}\big(c_\text{min} + \max(c_\text{sky},\, K_v |v_\text{rel,i}|),\ c_\text{min},\ c_\text{max}\big) $$

- `K_v|v_rel|`: 상대속도 적응항 — body-bounce 공진에서 stroke rate가 커지는 구간을 강하게 댐핑하여 공진 진폭을 억제(heave-capable).
- `c_sky`: 관측 가능한 모달(롤/피치) sprung 속도에 대한 skyhook.
...
베이스라인은 선회 중 제동으로 후륜 횡력을 잃고 **β가 30.5°까지 발산(스핀아웃)**한다. 본 설계는 슬립각 게이트가 열리며 K_lim의 강한 슬립각 안정화(보조 조향) + ESC 요모멘트가 작동, **β를 1.78°로 억제**한다. LTR도 0.681→0.315로 동반 감소.

![A7 sideslip](figures/a7_sideslip.png)
*Figure 4.1 — A7 차체 슬립각: OFF(스핀아웃) vs ON(LQR+ESC). ±5° 한계선 표시.*

흥미롭게도 ESC 요모멘트(차동 제동)를 꺼도 AFS만으로 A7이 안정화되었다(별도 진단). 즉 본 시나리오의 안정화는 **상태에 β를 포함한 LQR의 보조 조향이 주도**하고 차동 제동은 보조적이다.
...
초기 설계는 정상영역 LQR에도 yaw rate에 큰 페널티(`q_r_nom=80`)를 두었다. 그 결과 **폐루프 DLC(A1/D1)에서 제어기가 운전자가 의도한 yaw를 거슬러** ±8°(포화) 역조향을 내보냈고, Stanley 운전자가 이를 보상하려 더 세게 조향(±20°)하며 **운전자–제어기 limit cycle**이 발생했다. 시계열 진단(보조 조향 포화 + 운전자 조향 폭주)으로 원인을 특정하고, `q_r_nom`을 80→2로 낮추자:

- A1 sideSlip 4.22→**2.62**, D1 sideSlip 8.57→**3.46** (모두 목표 통과),
- A3 settling 2.04→**0.333** (절대 yaw 페널티가 응답을 과댐핑하던 것이 해소)

으로 **정량 점수가 35→57로 상승**했다. 교훈: 절대 yaw rate 페널티는 개루프(A3/A7)에는 댐핑으로 유익하나, 운전자가 능동적으로 yaw를 만드는 폐루프 추종에는 해롭다. 적분(`q_ξ`)으로 추종을 담당하게 하고 절대 yaw 페널티는 최소화하는 것이 통합 시나리오에 정합적이다.
...
- **A1 lateralDevMax (0/4)**: A1은 tireUtil 0.999, ay 0.94 g(μ=1)로 **타이어 포화** 상태다. 경로 오버슈트는 "더 꺾을 마찰이 없어서"이며, baseline(1.827 m)조차 0점 상한(1.4 m)을 초과한다. grip 한계라 제어 정교화로 회복 어렵다.
- **A1/D1 LTR 부분점**: LTR ∝ `ay = v²·(경로곡률)`이고 경로·속도가 고정이므로 LTR도 사실상 maneuver가 결정한다. β-안정화 강화·공격적 추종·ESC 감속(β 높을 때 제동) 3종을 실험했으나, 특히 한계영역 제동은 **마찰원(friction circle)으로 횡력을 빼앗아** 슬립각을 악화시킬 뿐 LTR을 줄이지 못했다.
...
횡방향 LQI(블렌딩) 파라미터, 종방향 ABS 파라미터, 수직 CDC 파라미터, coordinator 분배비를 `CTRL.*`에 추가하였으며, 모델 행렬용 차량 파라미터를 `CTRL.LAT.veh`로 전달한다(VEH 확정 후 복사). `LIM.*` 및 그 외 항목은 원본 유지. `SIM.solver`는 기본값 `ode45` 사용.
