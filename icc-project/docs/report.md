...
- **차량 파라미터**: `sim_params.m`은 `SIM.vehicleSet='bmw5_cm15'`로 BMW_5 실차값 로딩을 시도하나, 채점/재실행 환경에 CarMaker 차량 파일(`SIM.cm_vehicleFile`)이 없으면 generic C-segment 값으로 fallback한다($m=1500$ kg, $I_z=2500$ kg·m², $l_f=1.2$, $l_r=1.4$ m, $C_f=80000$, $C_r=85000$ N/rad). 본 `grade_report.json`은 해당 파일 부재(fallback 경고 확인)로 generic 값에서 산출되었으며, LQR 게인도 이 값에 맞춰 튜닝했으므로 재실행 시 동일 fallback이면 게인 전제가 보존된다. (헤더의 'BMW_5 14DOF'는 채점 *plant 차수·타이어 모델*을 가리키며, 위 *차량 파라미터셋*과는 별개 항목이다.) `ctrl_lateral`은 VEH를 직접 받지 않으므로 `sim_params.m`에서 `CTRL.LAT.veh`로 전달(VEH 확정 후 복사하여 plant와 동기).
...
시나리오별 breakdown 소계(부분점 합 — ASSIGNMENT §5.1의 binary view가 아니라 grade.m이 산출하는 KPI breakdown 합): A3 12/12, A4 10/10, A7 15/15, A1 9.6/15, D1 5.57/8, B1 5/10.
...
*Figure 4.3 — A1 DLC 궤적(OFF/ON/ref). 슬립각·LTR은 개선되나 경로 추종(dev)은 제어기에 경로오차 입력이 없어 제약(§5.3).*
...
남은 0점 항목은 *제어기법(LQR)의 한계가 아니라* **제어기 I/O 구조·고정 운전자·채점 규칙**의 한계다.

- **B1 stoppingDistance (0/5) — 규칙(하드코딩 금지) 한계**: 정지거리 = 1초 정속(27.78 m) + 제동거리다. 시나리오 제동은 t≥1 s에 시작하며, 제어기가 t<1 s에 선제동하려면 "지금이 B1"임을 인식해야 한다 = **시나리오 하드코딩(§3.1.4 금지)**. 일반 신호로는 근거가 없다(t<1 s엔 $v_x=v_\text{ref}$ 라 속도오차 0). 따라서 1초 정속이 불가피해 총 정지거리 ≥63.5 m인데, 채점 상한은 $40\times(1+50\%)=60$ m(grade.m, tol=50%)이므로 **규칙을 지키는 한 어떤 알고리즘도 0/5로 확정**된다. (참고: t=0부터 peak 마찰 $D_x=1.1$로 제동할 수 있다면 제동거리 ≈35.7 m, 총 ≈36 m로 만점이나, 이는 하드코딩을 전제하므로 본 과제에서 도달 불가.)
- **A1/D1 lateralDevMax (0/4, 0/2) — 제어기 입력 한계**: dev가 최대인 순간의 타이어 이용률은 A1 OFF 0.81 / ON 0.80으로 **grip 여유 ~20%가 남아 있어**(포화 0.999는 다른 시점) grip 한계가 아니다. 제약은 **`ctrl_lateral`이 위치·경로오차를 입력으로 받지 않는다**는 점이다(입력은 yaw·$\beta$·$v_x$ 뿐). 경로 추종은 고정 Stanley driver(read-only)의 몫이고, AFS는 yaw만 성형할 수 있어 dev를 직접 최소화할 신호·권한이 없다(추종 게인을 키워도 dev는 줄지 않고 sideSlip만 악화). NMPC/MPCC 등 경로추종 기법은 제어기가 경로오차를 받고 조향을 지휘함을 전제하므로 본 I/O에는 적용 불가하다. 실제로 본 설계는 dev를 baseline 대비 소폭 *악화*시킨다(A1 1.827→1.959 m): AFS의 yaw 성형이 운전자 경로를 미세 교란하기 때문이며, grade.m은 baseline 대비 악화 항목을 0점 처리하므로 dev는 어느 쪽이든 0/4·0/2로 귀결된다.
...
따라서 정량 실질 천장은 약 59점(70 − B1 stoppingDistance 5 − A1 dev 4 − D1 dev 2; 세 항목 모두 위에서 본 비가역 구조한계로 0점 확정)이며, 이는 **제어기 정교화로 못 넘는 게 아니라 I/O·규칙 구조** 때문이다. 현재 57.16은 그 ~97%다. (A1·D1 LTR 부분손실 ~1.8점도 일부는 maneuver·마찰원에 묶인 구조항이므로 실질 천장은 이보다 약간 더 낮을 수 있다.)

### 5.4 제어기법 선택의 정당성 — 왜 MPC가 아니라 LQR인가

MPC의 두 강점(제약 처리·preview)이 본 과제에서는 추가 점수로 이어지지 않는다:

- **dev**: 남은 dev 점수는 제어기에 경로오차 입력이 없어(§5.3) MPC라도 직접 줄일 수 없다 → MPC의 경로추종 강점이 무의미.
- **나머지 0점**: B1(규칙)·LTR(maneuver+마찰원)은 제어기 정교함과 무관하게 캡되어 MPC로도 동일하다.
- **비용·재현성**: MPC는 1 kHz QP 부하 + MPC/Optimization toolbox 의존 → TA 재실행 환경에 toolbox가 없으면 0점 리스크.

반면 LQR은 (a) AFS+ESC를 단일 비용함수로 최적 결합(MIMO), (b) 속도별 gain scheduling, (c) Hamiltonian Riccati 자체 구현으로 **toolbox 0의존(재현성 보장)**, (d) 구조 천장(~59점)의 **~97%(57.16)** 달성이라는 이점을 모두 제공한다. 즉 **MPC가 줄 추가 이득이 없는 상황에서 LQR이 더 단순·견고·재현가능**하므로 본 프로젝트에 최적이다. (제어기법이 아닌 *모델 차수* 측면에서, 롤 동역학을 포함한 고차 설계모델로 LTR을 직접 다루는 것이 유일하게 남은 실질 개선 방향이다.)
...
% coordinator (WLS allocation + 마찰원) — 제출 coordinator가 실제 사용하는 게인만 표기
CTRL.COORD.mu=1.0; CTRL.COORD.latReserve=0.7; CTRL.COORD.MzRef=4000;
CTRL.COORD.wFx=1; CTRL.COORD.wMz=1; CTRL.COORD.wU=1e-2;
% (sim_params의 brakeBiasF/ratioF/wLat/wLon/wVer/wEff는 simple-split 잔재로 WLS coordinator에서 미사용)
```
