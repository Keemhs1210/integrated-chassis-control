...
- **B1 stoppingDistance (0/5) — 규칙(하드코딩 금지) 한계**: 정지거리 = 1초 정속(27.78 m) + 제동거리다. 물리만 보면 t=0부터 peak 마찰($D_x=1.1$)로 제동 시 제동거리 ≈35.7 m → **총 ≈36 m < 40 m로 만점도 가능**하다. 그러나 시나리오 제동은 t≥1 s에 시작하며, 제어기가 t<1 s에 선제동하려면 "지금이 B1"임을 인식해야 한다 = **시나리오 하드코딩(§3.1.4 금지)**. 일반 신호로는 근거가 없다(t<1 s엔 $v_x=v_\text{ref}$ 라 속도오차 0). 즉 물리가 아니라 **하드코딩 금지 규칙** 때문에 1초 정속이 불가피 → 총 ≥63.5 m. 규칙을 어기지 않는 한 어떤 알고리즘도 불가.
...
따라서 정량 실질 천장은 약 61점(70 − B1 5 − A1 dev 4)이며, 이는 **제어기 정교화로 못 넘는 게 아니라 I/O·규칙 구조** 때문이다. 현재 57.16은 그 ~94%다.
...
반면 LQR은 (a) AFS+ESC를 단일 비용함수로 최적 결합(MIMO), (b) 속도별 gain scheduling, (c) Hamiltonian Riccati 자체 구현으로 **toolbox 0의존(재현성 보장)**, (d) 구조 천장(~61점)의 **94%(57.16)** 달성이라는 이점을 모두 제공한다. 즉 **MPC가 줄 추가 이득이 없는 상황에서 LQR이 더 단순·견고·재현가능**하므로 본 프로젝트에 최적이다. (제어기법이 아닌 *모델 차수* 측면에서, 롤 동역학을 포함한 고차 설계모델로 LTR을 직접 다루는 것이 유일하게 남은 실질 개선 방향이다.)
