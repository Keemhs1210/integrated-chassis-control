...
### 5.3 구조적 한계 — 회복 불가 항목 (데이터로 정정된 분석)

남은 0점 항목은 *제어기법(LQR)의 한계가 아니라* **제어기 I/O 구조·고정 운전자·채점 규칙**의 한계임을 데이터로 구분했다.
...
- **A1/D1 lateralDevMax (0/4, 0/2) — 제어기 입력 한계 (정정)**: *당초 'grip 한계'로 기술했으나 데이터로 정정한다.* dev가 최대인 순간의 타이어 이용률은 A1 OFF 0.81 / ON 0.80으로 **grip 여유 ~20%가 남아 있다**(포화 0.999는 다른 시점). 즉 물리적으로는 더 꺾을 여지가 있어 **grip 벽이 아니다**. 진짜 제약은 **`ctrl_lateral`이 위치·경로오차를 입력으로 받지 않는다**는 점이다(입력은 yaw·$\beta$·$v_x$ 뿐). 경로 추종은 고정 Stanley driver(read-only)의 몫이고, AFS는 yaw만 성형할 수 있어 dev를 직접 최소화할 신호·권한이 없다(공격적 추종 실험에서도 dev는 안 줄고 sideSlip만 악화). NMPC/MPCC 등 경로추종 기법은 *제어기가 경로오차를 받고 조향을 지휘*함을 전제하므로 본 I/O에는 적용 자체가 불가하다.
...
- **MPC**(제약·preview 기반): 한계영역의 입력 제약·preview를 더 잘 다룰 수 있다. 단 dev 개선은 *제어기에 경로오차 입력이 주어질 때만* 가능하므로(§5.3) 현재 I/O로는 MPC도 dev를 직접 못 줄인다. 또 MPC/Optimization toolbox 재현성 리스크가 있어 본 제출에서는 toolbox 비의존 LQR을 택했다.
